...
(() => {
  ...
  async function initPortfolio() {
    ...
    try {
      ...
      container.innerHTML =
        `<p class="exp-error">Impossible de charger les projets. Vérifie <code>data/portfolio.json</code>.</p>`;
      return;
    }
    ...
    const projects = data.map((item) => {
      ...
      const normalized = cats.map(normalizeCategoryLabel).filter(Boolean);
      return {
        ...item,
    ...
    const FILTERS = [
      ...
      ...dynamicFilters.map(([slug, label]) => ({ slug, label })),
    ];
    ...
    const renderCards = (filterSlug = "tous") => {
      ...
      if (CUSTOM_FILTERS[filterSlug]) {
        ...
        filtered =
          filterSlug === "tous"
            ? projects
            : projects.filter((p) => p.__catSlugs.includes(filterSlug));
      }
