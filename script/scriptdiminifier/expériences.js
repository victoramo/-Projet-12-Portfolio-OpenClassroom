 ...
 * ✅ fetch robuste + tri plus récent en premier
 */
...
async function createExperienceTimeline() {
  ...
  timeline.innerHTML = "<p class=\"exp-loading\">Chargement des expériences…</p>";
  ...
  if (!window.__EXPE_ESCAPE_BOUND__) {
    ...
    document.addEventListener("keydown", (event) => {
      ...
      if (btn) { expanded.hidden = true; btn.textContent = "En savoir plus"; }
    });
  }
}
...
function openModal(item, { modal, modalHeader, modalDetails }) {
  ...
  safeText(companyEl, safeMeta(item));
