...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initActiveNavLinks();
  initFloatThemeToggle();   // ✅ sync bouton thème responsive
  initResponsiveTopBar();   // ✅ hide/show barre responsive au scroll
});
...
function initResponsiveTopBar() {
  ...
  window.addEventListener("scroll", () => {
    ...
    requestAnimationFrame(() => {
      ...
      if (y <= 10)                  showBar(); // tout en haut → toujours visible
      else if (dir < -6)            showBar(); // remontée → affiche
      else if (dir > 6 && y > 80)  hideBar(); // descente → cache
